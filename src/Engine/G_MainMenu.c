...
void G_RenderCurrentMenuBackground(void)
{
    switch(currentMenu->ID)
    {
        case MENU_START:
        {
            SDL_Rect titleRect = {110, 10, tomentdatapack.menuAssets[M_ASSET_TITLE].texture->w, tomentdatapack.menuAssets[M_ASSET_TITLE].texture->h};
            R_BlitIntoScreenScaled(NULL, tomentdatapack.menuAssets[M_ASSET_TITLE].texture, &titleRect);
            break;
        }
    }
}
...
void G_RenderCurrentMenu(void)
{
    ...
    // Display cursor
    SDL_Rect cursorRect = {currentMenu->selectedElement->box.x - CURSOR_X_OFFSET, currentMenu->selectedElement->box.y, tomentdatapack.menuAssets[M_ASSET_SELECT_CURSOR].texture->w, tomentdatapack.menuAssets[M_ASSET_SELECT_CURSOR].texture->h};
    R_BlitIntoScreen(NULL, tomentdatapack.menuAssets[M_ASSET_SELECT_CURSOR].texture, &cursorRect);
}
